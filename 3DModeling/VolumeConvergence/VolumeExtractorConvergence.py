...
BASE = "/users/alanh//Documents/CBLResearch-github/VolumeConvergence"
...
def discover_cases(base):
    ...
    case_dirs = glob.glob(f'{base}/FinenessMeshes/case_*')
    case_nums = []
    for d in case_dirs:
...
def volume_extract(file_num, material_type, out_dir):
    ...
    ref_volume = pv.read(f'{BASE}/FinenessMeshes/case_{file_num}/mesh_{file_num}_volume.vtu')
    endo_ref = pv.read(f'{BASE}/FinenessMeshes/case_{file_num}/mesh_{file_num}_endo.vtp')
    ...
    files = sorted(glob.glob(
        f'{BASE}/results/case_{file_num}/result_*.vtu'))
...
def run_sweep(material_type, out_dir):
    ...
    if not cases:
        print(f"No case directories found under {BASE}/FinenessMeshes/", flush=True)
        return
